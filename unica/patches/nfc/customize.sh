...
# Add the following block to handle TARGET_CODENAME = a23xq
if [ "$TARGET_CODENAME" == "a23xq" ]; then
    FILE_PATH="$FW_DIR/${MODEL}_${REGION}/system/system/lib64/libnfc-st.so"
    if [ -f "$FILE_PATH" ]; then
        ADD_TO_WORK_DIR "system" "system/lib64/libnfc-st.so" 0 0 644 "u:object_r:system_lib_file:s0"
    else
        echo "Skipping libnfc-st.so as it is not found for $TARGET_CODENAME"
    fi
fi